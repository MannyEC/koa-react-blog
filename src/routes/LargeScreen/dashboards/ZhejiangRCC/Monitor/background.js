...
const trianglePos = {
  ...
  bottomLeft: Pos(centerPos.x - 120, centerPos.y + 140),
  bottomRight: Pos(centerPos.x + 120, centerPos.y + 140),
  ...
  nodeRight: Pos(centerPos.x + 220, centerPos.y + 140),
  nodeLeft: Pos(centerPos.x - 220, centerPos.y + 140),
};
...
function doubleCircle(ctx, centerX, centerY, radis, fromA, toA, withArrow) {
  ctx.beginPath();
  ctx.lineWidth = 5;
...
function circleArrow(ctx, centerX, centerY) {
  doubleCircle(ctx, centerX, centerY, 264, 4 / 12 * Math.PI, 6 / 12 * Math.PI, true);
  doubleCircle(ctx, centerX, centerY, 264, 7 / 12 * Math.PI, 8 / 12 * Math.PI, false);

  doubleCircle(ctx, centerX, centerY, 264, 12 / 12 * Math.PI, 14 / 12 * Math.PI, true);
  doubleCircle(ctx, centerX, centerY, 264, 15 / 12 * Math.PI, 16 / 12 * Math.PI, false);

  doubleCircle(ctx, centerX, centerY, 264, 20 / 12 * Math.PI, 22 / 12 * Math.PI, true);
  doubleCircle(ctx, centerX, centerY, 264, 23 / 12 * Math.PI, 24 / 12 * Math.PI, false);
}
...
function progressEffect(ctx, centerX, centerY, percent) {
  ...
  miniTriangle(ctx, linePointR.x, linePointR.y, 15, Math.PI * 11 / 6);

  const linePointL = effectPoint(trianglePos.leftBottom, trianglePos.leftTop, percent);
  miniTriangle(ctx, linePointL.x, linePointL.y, 15, Math.PI * 7 / 6);

  const linePointB = effectPoint(trianglePos.bottomRight, trianglePos.bottomLeft, percent);
  miniTriangle(ctx, linePointB.x, linePointB.y, 15, Math.PI * 1 / 2);
...
export const draw = (ctx, progress) => {
  ...
  hexagonItems(ctx, 100, 480, percent);
  hexagonItems(ctx, 520, 480, percent);
  circleArrow(ctx, 300, 350);
};
