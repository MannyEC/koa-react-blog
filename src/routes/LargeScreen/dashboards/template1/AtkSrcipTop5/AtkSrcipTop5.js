...
import classes from './AtkSrcipTop5.scss';
...
const getOption = (data, unit) => ({
  ...
  tooltip: {
    ...
    formatter(params) {
      ...
      return `${param.name} <br /> 攻击源流量峰值 ${humansize(param.value)}`;
    }
  },
  legend: {
    show: false
...
export default class AtkSrcipTop5 extends Component {
  static propTypes = {
      ...
      <PanelLayout>
        <div className={classes.container}>
          <div className={classes.header}>
            <span>{config && config.title ? config.title : '攻击源IP—流量峰值 TOP5'}</span>
          </div>
